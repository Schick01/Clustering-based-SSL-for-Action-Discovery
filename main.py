...
def main():
    ...
    if os.path.exists(path_features_resnet) and os.path.exists(path_labels_resnet):
        ...
        features_list = torch.load(path_features_resnet)
        labels_list = torch.load(path_labels_resnet)
        print(f"Feature caricate con forma: {features_list.shape}")
        print(f"Etichette caricate: {len(labels_list)}")
    else:
        ...
        torch.save(features_list, 'data/features_resnet18.pt')
        torch.save(labels_list, 'data/labels_resnet18.pt')
    ...
    dataset_videomae = VideoKineticsDataset(transform=videomae_processor)
    ...
    if os.path.exists(path_features_videomae) and os.path.exists(path_labels_videomae):
        print("Feature e etichette già salvate. Caricamento...")
        features_list_videomae = torch.load(path_features_videomae)
        labels_list_videomae = torch.load(path_labels_videomae)
        
        print(f"Feature caricate con forma: {features_list_videomae.shape}")
        print(f"Etichette caricate: {len(labels_list_videomae)}")
    else:
        print("Feature e etichette non trovate. Estrazione in corso...")
        model_videomae = VideoMAEFeatureExtractor()

        features_list_videomae = []
        labels_list_videomae = []
        ...
        with torch.no_grad():
            for video, label in tqdm(loader_videomae):
                ...
                features_list_videomae.append(feature)
                labels_list_videomae.extend(label)

        print("Salvataggio delle feature e delle etichette...")
        features_list_videomae = torch.cat(features_list_videomae)
        torch.save(features_list_videomae, 'data/features_videomae.pt')
        torch.save(labels_list_videomae, 'data/labels_videomae.pt')
        
        print(f"Finito! Feature salvate con forma: {features_list_videomae.shape}")
        print(f"Etichette salvate: {len(labels_list_videomae)}")
