...
Tutti gli obiettivi minimi della traccia sono raggiunti, e la domanda centrale ha una risposta netta: **il clustering iterativo funziona**, converge autonomamente e migliora le proprie partizioni su entrambi i backbone, **ma il suo effetto dipende da tre condizioni misurate**: la maturità delle feature di partenza (decide *cosa* il loop può fare: raffinare o costruire), la scala dei dati (decide *se* si innesca) e la calibrazione del regime di training (decide se raffina o memorizza).

I limiti: **(i)** K=10 assume noto il numero di azioni, realistico per la valutazione ma generoso per lo scenario di discovery puro; **(ii)** anche il dataset esteso resta 2–3 ordini di grandezza sotto la scala nativa di DeepCluster, e la saturazione osservata potrebbe non essere l'ultima parola; **(iii)** il mean pooling temporale scarta la dinamica del movimento, penalizzando proprio le classi che più ne avrebbero bisogno; **(iv)** l'augmentation è solo spaziale. Le direzioni future più promettenti: un segnale contrastivo o temporale accanto alle pseudo-label, pooling temporali più espressivi, stima non supervisionata di K.
